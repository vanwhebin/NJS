...
from nonebot.plugin.on import on_command, on_keyword, on_message, on_notice
...
attack = on_keyword(
    curse, rule=to_me(), block=True, priority=1
)  # ban 人的响应器, curse 是关键词列表
remove_CD = on_command(
    "remove_qq",
    aliases={"rm_qq"},
...
setu_switch = on_command(
    "切换图片api", rule=to_me(), priority=1, permission=SUPERUSER, block=True
)
api_check = on_command(
    "查看当前api", aliases={"查看api"}, priority=5, block=True, rule=to_me()
)  # 普通用户也可以查询
poke = on_notice(rule=to_me(), block=False)  # 戳一戳
# -------------------------------ChatGPT------------------------------
refresh = on_command(
    "api刷新对话", aliases={"api刷新会话"}, priority=50, block=True, rule=to_me()
)  # 重新开始 ChatGPT 的对话
refresh = on_command(
    "token刷新对话", aliases={"token刷新会话"}, block=True, rule=to_me(), priority=10
)
...
async def handle_receive(event: MessageEvent):
    ...
    await attack.send(
        message=f"{random.choice(attack_sendmessage)}" + MessageSegment.image(img),
        at_sender=True,
    )
...
async def _(switch_msg: Message = CommandArg()):
    ...
    finish_msg = (
        f"切换成功, 当前智能回复api为{api_num}".replace("1", "小爱同学模式1")
        .replace("2", "青云客")
        .replace("3", "ChatGPT(使用Token)")
        .replace("4", "ChatGPT(使用API)")
        .replace("0", "小爱同学模式2")
    )
...
async def _():
    ...
    await setu_switch.send(
        message=f"切换成功, 当前戳一戳图片api为{'MirlKoi' if setu_flag else 'Pixiv'}",
        at_sender=True,
    )
...
async def check():
    check_msg = (
        f"当前图片 api 为 {setu_flag};\n当前聊天 api 为 {api_num}".replace(
            "1", "小爱同学模式1"
        )
        .replace("2", "青云客")
        .replace("3", "ChatGPT(使用Token)")
    ...
    if result == None:
        if api_num in [0, 1]:
            ...
            if api_num == 1:
                ...
                with open(
                    "./src/plugins/nonebot_plugin_smart_reply/voice.mp3", "wb"
                ) as f:
                    f.write(response.content)
                await ai.finish(
                    message=MessageSegment.record(
                        file="./src/plugins/nonebot_plugin_smart_reply/voice.mp3"
                    ).record(file=voice)
                )
    ...
    if event.is_tome:
        ...
        if probability < 0.20:
            if setu_flag:
                ...
                res = httpx.get(
                    url="https://dev.iw233.cn/api.php?sort=pc&type=json",
                ...
                # pic_url = res["pic"][0]
                message = (
                    "别戳了别戳了, 这张图给你了, 让我安静一会儿, 60秒后我要撤回\n"
                    + MessageSegment.image(file=pic_url)
                )
            else:
                pic = await get_setu()
                message = (
                    "别戳了别戳了, 这张setu给你了, 让我安静一会儿, 60秒后我要撤回\n"
                    + Message(pic[1])
                    + Message(pic[0])
                )
            ...
            try:
                await poke.send(
                    MessageSegment.record(
                        Path(aac_file_path) / random.choice(aac_file_list)
                    )
                )
            except:
