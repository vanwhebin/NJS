...
nickname = list(get_driver().config.nickname)[0] if len(get_driver().config.nickname) else "nonebot-plugin-stable-diffusion-diao"
...
class Config(BaseSettings):
    ...
    novelai_command_start: set = {"绘画", "咏唱", "约稿", "aidraw", "画", "绘图", "AI绘图", "ai绘图"}
    ...
    novelai_SuperRes_generate_payload: dict = {"upscaling_resize": 1.2, "upscaler_1": "Lanczos", "upscaler_2": "R-ESRGAN 4x+ Anime6B", "extras_upscaler_2_visibility": 0.7}  # 超分倍率, 为长宽分辨率各X1.2  # 第一次超分使用的方法  # 第二次超分使用的方法  # 第二层upscaler力度  # 以上为个人推荐值
    ...
    novelai_ControlNet_payload: list = [
        ...
        {"controlnet_units": [{"input_image": "", "module": "scribble_hed", "model": "control_v11p_sd15_scribble [d4ba51ff]", "weight": 1, "lowvram": "false", "processor_res": novelai_size * 1.5, "threshold_a": 100, "threshold_b": 250}]},
    ]

    novelai_cndm: dict = {"controlnet_module": "canny", "controlnet_processor_res": novelai_size, "controlnet_threshold_a": 100, "controlnet_threshold_b": 250}
    ...
    novelai_pic_audit_api_key: dict = {"SECRET_KEY": "", "API_KEY": ""}  # 你的百度云API Key
